...
void FlocksVisualsTable::Init(FMeshRendererData InMeshRendererData, UInstancedStaticMeshComponent* InISMComponent)
{
	...
	{
		Data[i].BoidCustomData.SetNum(NumCustomDataValues);
	}
...
TArray<unsigned short> FlocksVisualsTable::AddBoid(int32 Amount, BOID_HANDLE* Handles, const FAddBoidData* InData)
{
	...
	TArray<FlocksAnimationTable::FAddBoidData> AnimTableBoidData;
	AnimTableBoidData.SetNum(MAX_BOIDS);
	...
	ParallelFor(Amount, [this, &AddedIndices, &NumBoidsAdded, &AnimTableBoidData, Handles, InData](int32 i) {
		...
		{
			memset(Data[TableIndex].BoidCustomData.GetData(), 0, sizeof(float) * NumCustomDataValues);

			int32 MinCustomDataValues = FMath::Min(NumCustomDataValues, InData[i].CustomDataValues.Num());
			memcpy(Data[TableIndex].BoidCustomData.GetData(), InData[i].CustomDataValues.GetData(), sizeof(float) * MinCustomDataValues);
	...
	{
		AnimationTable->AddBoid(Amount, Handles, AnimTableBoidData.GetData());
	}
...
void FlocksVisualsTable::RemoveBoid(int32 Amount, BOID_HANDLE* Handles)
{
	...
	for (int32 i = 0; i < Amount; ++i)
	{
		...
		if (Index != INVALID_BOID_HANDLE_INDEX)
		{
			...
			memcpy(Data[Index].BoidCustomData.GetData(), Data[Size()].BoidCustomData.GetData(), sizeof(float) * NumCustomDataValues);
		}
	}
	Unlock();
