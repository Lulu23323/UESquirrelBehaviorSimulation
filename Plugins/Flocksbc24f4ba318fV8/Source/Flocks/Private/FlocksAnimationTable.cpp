...
TArray<uint16> FlocksAnimationTable::AddBoid(int32 Amount, BOID_HANDLE* Handles, const FAddBoidData* InData)
{
...
void FlocksAnimationTable::Update(float DeltaSeconds)
{
	...
	ParallelFor(Size(), [this, DeltaSeconds](int32 b)
	{
		...
		if (Data[b].AnimTime >= Data[b].EndTime)
		{
			...
			if (Data[b].CurrentLoops >= Data[b].TargetLoops && Data[b].TargetLoops != -1)
			{
...
void FlocksAnimationTable::PlayOverrideAnimation(BOID_HANDLE Boid, const FName& Animation, float StartOffset, int32 NumLoops)
{
	PlayAnimation(Boid, Animation, StartOffset, NumLoops);
}

void FlocksAnimationTable::ClearAnimationOverride(BOID_HANDLE Boid)
{
	uint16 Index = Find(Boid);
	switch (Data[Index].AnimationMode)
	{
	case EFlocksAnimationMode::LoopSingle:
		PlayAnimation(Boid, Data[Index].LoopSingleAnimationName, 0.0f, 1);
		break;

	case EFlocksAnimationMode::PlayVarious:
		SelectNewVariousAnimation(Boid, 0.0f);
		break;
	}
}
