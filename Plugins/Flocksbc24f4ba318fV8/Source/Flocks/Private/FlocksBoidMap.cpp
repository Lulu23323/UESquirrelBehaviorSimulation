...
{
	Data.CellBoidCount.SetNum(TOTAL_CELLS);
	Data.CellOffsetList.SetNum(TOTAL_CELLS);
...
void FlocksBoidMap::Update(float DeltaSeconds)
{
    ...
    TArray<FVector3f> BoidPositions;
	BoidPositions.SetNum(MAX_BOIDS);
...

	{
		...
		for (uint16 i = 0; i < World->StateTable->Size(); ++i)
			{
				...
				uint16 Cell = GetFlatCellIndex(CellVector);
...

	{
		...
		uint32 LastCell = UINT32_MAX;
		for (uint32 i = 0; i < World->StateTable->Size(); ++i)
		{
			uint32 Cell = (Data.SortedCellList[i] & 0Xffff0000) >> 16;
			if (LastCell != Cell)
			{
...

	{
		...
		ParallelFor(TOTAL_CELLS, [this](uint16 Index)
			{
				uint16 Offset = Data.CellOffsetList[Index];
				uint16 Count = Data.CellBoidCount[Index];

				if (Offset != UINT16_MAX)
				{
					while (Count > 0)
					{
						uint16 Element = Offset + FMath::RandRange(0, (Count - 1));

						uint16 Temp = Data.SortedCellList[Element];
...
uint32 FlocksBoidMap::GetFlatCellIndex(FIntVector CellIndex) const
{
...
void FlocksBoidMap::Reset()
{
	for(int32 i = 0; i < TOTAL_CELLS; i++)
	{
		Data.CellBoidCount[i] = 0;
		Data.CellOffsetList[i] = TNumericLimits<uint32>::Max();
	}
}
