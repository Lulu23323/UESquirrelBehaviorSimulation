...
uint16 FlocksVolumeInfoTable::AddVolume(VOLUME_HANDLE Volume, const FAddVolumeData& AddVolumeData)
{
...
void FlocksVolumeInfoTable::SetVolumeType(VOLUME_HANDLE Volume, const FlocksVolumeTypes Type)
{
	...
	{
		Lock();
		Data[Index].VolumeType = static_cast<uint32>(Type);
		Unlock();
	}
}
...
void FlocksVolumeInfoTable::SetVolumeInfluenceGroups(VOLUME_HANDLE Volume, int32 Groups)
{
	...
	{
		Lock();
		Data[Index].VolumeInfluencesGroups = static_cast<uint32>(Groups);
		Unlock();
	}
}
