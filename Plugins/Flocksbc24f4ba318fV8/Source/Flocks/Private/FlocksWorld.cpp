...
TWeakObjectPtr<AFlocksWorld> AFlocksWorld::WorldInst = nullptr;

AFlocksWorld::AFlocksWorld()
{	
	DefaultBoidSettings.SetNum(MAX_GROUPS);
	DefaultMeshSettings.SetNum(MAX_MESHES);
	BoidsPendingDelete.SetNum(MAX_BOIDS);
	ISMComponents.SetNum(MAX_MESHES);
	VisualsTable.SetNum(MAX_MESHES);
	GroupData.SetNum(MAX_GROUPS);
	...
	FlockShader = new FlocksShaderManager();
...
void AFlocksWorld::UpdateBoids(float DeltaTime)
{
	...
	else
	{
		...
		FlocksVolumeInfoTable::FTableData* VolumeTableData = VolumeInfoTable->Lock();
		TArray<FlocksStateTable::FTableData> OutData;
		OutData.SetNum(MAX_BOIDS);
		memcpy(OutData.GetData(), StateTableData, sizeof(FlocksStateTable::FTableData) * MAX_BOIDS);
		...
		memcpy(StateTableData, OutData.GetData(), sizeof(FlocksStateTable::FTableData) * MAX_BOIDS);
		StateTable->Unlock();
		VolumeInfoTable->Unlock();
		BoidMap->Unlock();
		UnlockGroupData();
...
TArray<BOID_HANDLE> AFlocksWorld::BatchAddBoids(int32 Amount, int32 Group)
{
	...
	TArray<FAddBoidData> BoidDatas;
	BoidDatas.SetNum(Amount);
	...
	OutBoids = AddBoid_Internal(Amount, BoidDatas.GetData());
	return OutBoids;
}
...
TArray<BOID_HANDLE> AFlocksWorld::AddBoid_Internal(int32 Amount, const FAddBoidData* AddBoidData)
{
	...
	TArray<int32> NumVisualsBoids;
	NumVisualsBoids.SetNum(MAX_MESHES);
	...
	TArray<TArray<BOID_HANDLE>> VisualsBoids;
	TArray<TArray<FlocksVisualsTable::FAddBoidData>> VisualsBoidData;

	VisualsBoids.SetNum(MAX_MESHES);
	VisualsBoidData.SetNum(MAX_MESHES);

	for (int32 i = 0; i < MAX_MESHES; ++i)
	{
		VisualsBoids[i].SetNum(Amount);
		VisualsBoidData[i].SetNum(Amount);
	}

	TArray<FlocksStateTable::FAddBoidData> StateBoidData;
	StateBoidData.SetNum(Amount);
	...
	// Add to tables
	StateTable->AddBoid(ValidHandles.Num(), ValidHandles.GetData(), StateBoidData.GetData());
	for (int32 i = 0; i < MAX_MESHES; ++i)
	{
		VisualsTable[i]->AddBoid(NumVisualsBoids[i], VisualsBoids[i].GetData(), VisualsBoidData[i].GetData());
	}
...
void AFlocksWorld::BatchRemoveBoids(int32 Amount)
{
	...
	TArray<BOID_HANDLE> AllBoidHandles;
	AllBoidHandles.SetNum(TotalBoids);
	...
	TArray<BOID_HANDLE> HandlesToRemove;
	HandlesToRemove.SetNum(Amount);
	...
	RemoveBoid_Internal(Amount, HandlesToRemove.GetData());
}

void AFlocksWorld::BatchRemoveBoids(const TArray<BOID_HANDLE>& Handles)
{
	RemoveBoid_Internal(Handles.Num(), Handles.GetData());
}

void AFlocksWorld::RemoveBoid_Internal(int32 Amount, const BOID_HANDLE* Handles)
{
	FlushRenderingCommands();

	int32 NumStateTableHandles = 0;
	TArray<BOID_HANDLE> StateTableHandles;
	StateTableHandles.SetNum(Amount);

	TArray<int32> NumVisualsTableHandles;
	TArray<TArray<BOID_HANDLE>> VisualsTableHandles;
	NumVisualsTableHandles.SetNum(MAX_MESHES);
	VisualsTableHandles.SetNum(MAX_MESHES);
	
	for (int32 i = 0; i < MAX_MESHES; ++i)
	{
		NumVisualsTableHandles[i] = 0;
		VisualsTableHandles[i].SetNum(Amount);
	}
	...
	// Remove from Tables
	StateTable->RemoveBoid(NumStateTableHandles, StateTableHandles.GetData());

	for (int32 i = 0; i < MAX_MESHES; ++i)
	{
		VisualsTable[i]->RemoveBoid(NumVisualsTableHandles[i], VisualsTableHandles[i].GetData());
	}
}
