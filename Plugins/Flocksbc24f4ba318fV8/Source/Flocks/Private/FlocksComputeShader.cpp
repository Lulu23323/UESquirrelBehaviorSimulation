...
public:
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint32>, SortedCellList)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint32>, CellOffsetList)
		SHADER_PARAMETER_UAV(RWStructuredBuffer<uint32>, CellBoidCount)
...
inline void* FlocksLockBuffer(FRHICommandListImmediate& RHICmdList, FFlocksBufferRHIRef Buffer, uint32 Size, EResourceLockMode LockMode)
{
...
void CreateBufferAndUAV(uint32 Stride, uint32 AllocationSizeBytes, FFlocksBufferRHIRef& Buffer, FUnorderedAccessViewRHIRef& UAV)
{
...
void UpdateBufferFromData(FRHICommandListImmediate& RHICmdList, void* Data, uint32 CopySizeBytes, FFlocksBufferRHIRef& Buffer)
{
	if (CopySizeBytes == 0)
		return;
...
void FlocksShaderManager::Initialize(AFlocksWorld* InWorld)
{
	World = InWorld;
	...
	ENQUEUE_RENDER_COMMAND(CreateFlocksBuffers)([this](FRHICommandListImmediate& RHICmdList)
		{
			CreateBufferAndUAV(sizeof(uint32), sizeof(uint32) * MAX_BOIDS, SortedCellListBuffer, SortedCellListUAV);
			CreateBufferAndUAV(sizeof(uint32), sizeof(uint32) * TOTAL_CELLS, CellOffsetListBuffer, CellOffsetListUAV);
			CreateBufferAndUAV(sizeof(uint32), sizeof(uint32) * TOTAL_CELLS, CellBoidCountBuffer, CellBoidCountUAV);
...
void FlocksShaderManager::Shutdown()
{
	World = nullptr;
}
...
void FlocksShaderManager::Execute_RenderThread(FRHICommandListImmediate& RHICmdList, float CurrentTime)
{
	...
	UpdateBufferFromData(RHICmdList, BoidMapData->SortedCellList.GetData(), sizeof(uint32) * MAX_BOIDS, SortedCellListBuffer);
	UpdateBufferFromData(RHICmdList, BoidMapData->CellOffsetList.GetData(), sizeof(uint32) * TOTAL_CELLS, CellOffsetListBuffer);
	UpdateBufferFromData(RHICmdList, BoidMapData->CellBoidCount.GetData(), sizeof(uint32) * TOTAL_CELLS, CellBoidCountBuffer);
