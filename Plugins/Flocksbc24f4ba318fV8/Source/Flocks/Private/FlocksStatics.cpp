...
void UFlocksStatics::BatchRemoveBoidsByHandles(const TArray<FFlocksBoidBlueprintHandle>& Handles)
{
#if WITH_EDITOR
	VALIDATE_FLOCKS_WORLD();
#endif

	TArray<BOID_HANDLE> NativeHandles;
	NativeHandles.SetNum(Handles.Num());

	ParallelFor(Handles.Num(), [&NativeHandles, Handles](int32 Index)
	{
		NativeHandles[Index] = Handles[Index].Handle;
	});

	AFlocksWorld::WorldInst->BatchRemoveBoids(NativeHandles);
}
...
void UFlocksStatics::SetMaxHealthForBoid(const FFlocksBoidBlueprintHandle& Boid, float NewMaxHealth, bool KeepHealthPercent)
{
	...
	uint16 Index = StateTable->Find(Boid.Handle);
...
void UFlocksStatics::SetAnimRateForBoid(const FFlocksBoidBlueprintHandle& Boid, float NewAnimRate)
{
	...
	FlocksStateTable::FTableData* StateTableData = StateTable->Lock();
	uint16 StateTableIndex = StateTable->Find(Boid.Handle);
...
void UFlocksStatics::SetMaxHealthForAllBoids(float NewMaxHealth, bool bKeepHealthPercent)
{
	...
	TArray<float> OldHealths;
	TArray<float> NewHealths;
	OldHealths.SetNum(MAX_BOIDS);
	NewHealths.SetNum(MAX_BOIDS);
	ParallelFor(StateTable->Size(), [StateTableData, &OldHealths, &NewHealths, NewMaxHealth, bKeepHealthPercent](uint16 Index)
		{
...
void UFlocksStatics::SetSpeedForAllBoids(float NewSpeed)
{
	...
	ParallelFor(StateTable->Size(), [&StateTableData, NewSpeed](uint16 Index)
		{
			StateTableData[Index].Speed = NewSpeed;
		});
...
void UFlocksStatics::SetTurningSpeedForAllBoids(float NewTurningSpeed)
{
	...
	ParallelFor(StateTable->Size(), [&StateTableData, NewTurningSpeed](uint16 Index)
		{
			StateTableData[Index].Turning = NewTurningSpeed;
		});
...
void UFlocksStatics::SetScaleForAllBoids(float NewScale)
{
	...
	ParallelFor(StateTable->Size(), [&StateTableData, NewScale](uint16 Index)
		{
			StateTableData[Index].Scale = NewScale;
		});
...
void UFlocksStatics::SetAnimRateForAllBoids(float NewAnimRate)
{
	...
	for (int32 i = 0; i < MAX_MESHES; ++i)
	{
		...
		FlocksAnimationTable::FTableData* AnimationTableData = AnimationTable->Lock();
		ParallelFor(AnimationTable->Size(), [AnimationTableData, NewAnimRate](uint16 Index)
		{
...
void UFlocksStatics::RandomizeSpeedForAllBoids(float MinSpeed, float MaxSpeed)
{
	...
	ParallelFor(StateTable->Size(), [&StateTableData, MinSpeed, MaxSpeed](uint16 Index)
		{
			StateTableData[Index].Speed = FMath::RandRange(MinSpeed, MaxSpeed);
		});
...
void UFlocksStatics::RandomizeTurningSpeedForAllBoids(float MinTurningSpeed, float MaxTurningSpeed)
{
	...
	ParallelFor(StateTable->Size(), [&StateTableData, MinTurningSpeed, MaxTurningSpeed](uint16 Index)
		{
			StateTableData[Index].Turning = FMath::RandRange(MinTurningSpeed, MaxTurningSpeed);
		});
...
void UFlocksStatics::RandomizeScaleForAllBoids(float MinScale, float MaxScale)
{
	...
	ParallelFor(StateTable->Size(), [&StateTableData, MinScale, MaxScale](uint16 Index)
		{
			StateTableData[Index].Scale = FMath::RandRange(MinScale, MaxScale);
		});
...
void UFlocksStatics::RandomizeAnimRateForAllBoids(float MinAnimRate, float MaxAnimRate)
{
	...
	for (int32 i = 0; i < MAX_MESHES; ++i)
	{
		...
		FlocksAnimationTable::FTableData* AnimationTableData = AnimationTable->Lock();
		ParallelFor(AnimationTable->Size(), [AnimationTableData, MinAnimRate, MaxAnimRate](uint16 Index)
			{
...
void UFlocksStatics::UpdateSettingsForAllBoids(const FDefaultBoidSettings& Settings, bool bKeepHealthPercent)
{
	...
	OldHealths.SetNum(MAX_BOIDS);
	NewHealths.SetNum(MAX_BOIDS);
	ParallelFor(StateTable->Size(), [&StateTableData, &OldHealths, &NewHealths, StateTable, Settings, bKeepHealthPercent](uint16 Index)
		{
...
void UFlocksStatics::SetMaxHealthForAllBoidsInGroup(int32 Group, float NewMaxHealth, bool bKeepHealthPercent)
{
	...
	OldHealths.SetNum(MAX_BOIDS);
	NewHealths.SetNum(MAX_BOIDS);
	ParallelFor(StateTable->Size(), [StateTableData, &OldHealths, &NewHealths, Group, NewMaxHealth, bKeepHealthPercent](uint16 Index)
		{
...
void UFlocksStatics::SetSpeedForBoidsInGroup(int32 Group, float NewSpeed)
{
	...
	ParallelFor(StateTable->Size(), [&StateTableData, Group, NewSpeed](uint16 Index)
		{
			if (StateTableData[Index].Group == Group)
			{
				StateTableData[Index].Speed = NewSpeed;
...
void UFlocksStatics::SetTurningSpeedForBoidsInGroup(int32 Group, float NewTurningSpeed)
{
	...
	ParallelFor(StateTable->Size(), [&StateTableData, Group, NewTurningSpeed](uint16 Index)
		{
			if (StateTableData[Index].Group == Group)
			{
				StateTableData[Index].Turning = NewTurningSpeed;
...
void UFlocksStatics::SetScaleForBoidsInGroup(int32 Group, float NewScale)
{
	...
	ParallelFor(StateTable->Size(), [&StateTableData, Group, NewScale](uint16 Index)
		{
			if (StateTableData[Index].Group == Group)
			{
				StateTableData[Index].Scale = NewScale;
...
void UFlocksStatics::SetAnimRateForBoidsInGroup(int32 Group, float NewAnimRate)
{
	...
	FlocksStateTable* StateTable = World->StateTable;
	FlocksStateTable::FTableData* StateTableData = StateTable->Lock();
	TArray<FlocksAnimationTable*> AnimationTables;
	TArray<FlocksAnimationTable::FTableData*> AnimationTableData;
	AnimationTables.SetNum(MAX_MESHES);
	AnimationTableData.SetNum(MAX_MESHES);
	...
	ParallelFor(StateTable->Size(), [StateTable, &StateTableData, AnimationTables, &AnimationTableData, Group, NewAnimRate](uint16 Index)
		{
			if (StateTableData[Index].Group == Group)
			{
...
void UFlocksStatics::RandomizeSpeedForAllBoidsInGroup(int32 Group, float MinSpeed, float MaxSpeed)
{
	...
	ParallelFor(StateTable->Size(), [&StateTableData, Group, MinSpeed, MaxSpeed](uint16 Index)
		{
			if (StateTableData[Index].Group == Group)
			{
				StateTableData[Index].Speed = FMath::RandRange(MinSpeed, MaxSpeed);
...
void UFlocksStatics::RandomizeTurningSpeedForAllBoidsInGroup(int32 Group, float MinTurningSpeed, float MaxTurningSpeed)
{
	...
	ParallelFor(StateTable->Size(), [&StateTableData, Group, MinTurningSpeed, MaxTurningSpeed](uint16 Index)
		{
			if (StateTableData[Index].Group == Group)
			{
				StateTableData[Index].Turning = FMath::RandRange(MinTurningSpeed, MaxTurningSpeed);
...
void UFlocksStatics::RandomizeScaleForAllBoidsInGroup(int32 Group, float MinScale, float MaxScale)
{
	...
	ParallelFor(StateTable->Size(), [&StateTableData, Group, MinScale, MaxScale](uint16 Index)
		{
			if (StateTableData[Index].Group == Group)
			{
				StateTableData[Index].Scale = FMath::RandRange(MinScale, MaxScale);
...
void UFlocksStatics::RandomizeAnimRateForAllBoidsInGroup(int32 Group, float MinAnimRate, float MaxAnimRate)
{
	...
	ParallelFor(StateTable->Size(), [StateTable, &StateTableData, AnimationTables, &AnimationTableData, Group, MinAnimRate, MaxAnimRate](uint16 Index)
		{
			if (StateTableData[Index].Group == Group)
			{
				BOID_HANDLE Boid = StateTable->Get(Index);
...
void UFlocksStatics::UpdateSettingsForAllBoidsInGroup(int32 Group, const FDefaultBoidSettings& Settings, bool bKeepHealthPercent)
{
	...
	OldHealths.SetNum(MAX_BOIDS);
	NewHealths.SetNum(MAX_BOIDS);
	ParallelFor(StateTable->Size(), [&StateTableData, &OldHealths, &NewHealths, Group, StateTable, Settings, bKeepHealthPercent](uint16 Index)
		{
			OldHealths[Index] = 0.0f;
			NewHealths[Index] = 0.0f;
...
void UFlocksStatics::UpdateBoidDefaultMaxHealthForGroup(int32 Group, float NewMaxHealth, EUpdateBoidDefaultHealthMode UpdateHealthMode)
{
	...
	if (UpdateHealthMode != EUpdateBoidDefaultHealthMode::LeaveExistingMaxHealth)
	{
		...
		ParallelFor(StateTable->Size(), [&StateTableData, &OldHealths, &NewHealths, Group, StateTable, NewMaxHealth, bKeepHealthPercent](uint16 Index)
			{
				OldHealths[Index] = 0.0f;
				NewHealths[Index] = 0.0f;
...
void UFlocksStatics::UpdateBoidDefaultAnimRateForMesh(int32 MeshIndex, float MinAnimRate, float MaxAnimRate, EUpdateBoidDefaultsMode UpdateMode)
{
	...
	if(UpdateMode != EUpdateBoidDefaultsMode::LeaveExistingBoidValues)
	{
		...
		ParallelFor(AnimationTable->Size(), [&AnimationTableData, UpdateMode, MinAnimRate, MaxAnimRate](uint16 Index)
		{
			switch (UpdateMode)
			{
				case EUpdateBoidDefaultsMode::ClampExistingBoidValues:
...
void UFlocksStatics::UpdateBoidDefaultSettingsForGroup(int32 Group, const FDefaultBoidSettings& Settings, EUpdateBoidDefaultsMode UpdateMode, EUpdateBoidDefaultHealthMode UpdateHealthMode)
{
	...
	{
	case EUpdateBoidDefaultsMode::RandomizeExistingBoidValues:
		ParallelFor(StateTable->Size(), [StateTableData, Group, Settings](uint16 Index)
			{
				if (StateTableData[Index].Group == Group)
				{
					StateTableData[Index].Speed = FMath::RandRange(Settings.MinSpeed, Settings.MaxSpeed);
	...
	if (UpdateHealthMode != EUpdateBoidDefaultHealthMode::LeaveExistingMaxHealth)
	{
		...
		bool bKeepHealthPercent = UpdateHealthMode == EUpdateBoidDefaultHealthMode::KeepHealthPercent;
		ParallelFor(StateTable->Size(), [&StateTableData, &OldHealths, &NewHealths, Group, Settings, bKeepHealthPercent](uint16 Index)
			{
				OldHealths[Index] = 0.0f;
				NewHealths[Index] = 0.0f;
...
bool UFlocksStatics::GetAnimNameFromIndex(UFlocksAnimationData* AnimData, int32 Index, FName& OutName)
{
	OutName = NAME_None;
	...
	if(AnimData->Animations.IsValidIndex(Index))
	{
		OutName = AnimData->Animations[Index].AnimationName;
		return true;
	}
...
void UFlocksStatics::PlayOverrideAnimationForBoid(const FFlocksBoidBlueprintHandle& Boid, const FName AnimationName, float StartTime, int32 NumLoops)
{
#if WITH_EDITOR
	VALIDATE_FLOCKS_WORLD_AND_BOID_HANDLE(Boid);
#endif
	...
	FlocksStateTable* StateTable = World->StateTable;
	FlocksStateTable::FTableData* StateTableData = StateTable->Lock();
	uint16 BoidIndex = StateTable->Find(Boid.Handle);
	int32 MeshIndex = StateTableData[BoidIndex].MeshIndex;

	FlocksAnimationTable* AnimationTable = World->VisualsTable[MeshIndex]->AnimationTable;
	AnimationTable->Lock();
	AnimationTable->PlayOverrideAnimation(Boid.Handle, AnimationName, StartTime, NumLoops);
	AnimationTable->Unlock();

	StateTable->Unlock();
}

void UFlocksStatics::PlayOverrideAnimationForBoids(const TArray<FFlocksBoidBlueprintHandle>& Boids, const FName AnimationName, float StartTime, int32 NumLoops)
{
#if WITH_EDITOR
	VALIDATE_FLOCKS_WORLD();
#endif
	...
	{
		AnimationTables[i] = World->VisualsTable[i]->AnimationTable;
		World->VisualsTable[i]->AnimationTable->Lock();
	}

	ParallelFor(Boids.Num(), [StateTable, &StateTableData, AnimationTables, Boids, AnimationName, StartTime, NumLoops](uint16 Index)
	{
		BOID_HANDLE Boid = Boids[Index].Handle;
		uint16 BoidIndex = StateTable->Find(Boid);

		if(BoidIndex != INVALID_BOID_HANDLE_INDEX)
		{
			int32 MeshIndex = StateTableData[BoidIndex].MeshIndex;

			if(AnimationTables.IsValidIndex(MeshIndex))
			{
				FlocksAnimationTable* AnimationTable = AnimationTables[MeshIndex];
				AnimationTable->PlayOverrideAnimation(Boid, AnimationName, StartTime, NumLoops);
			}
		}
	});
...
void UFlocksStatics::PlayOverrideAnimationForAllBoids(const FName AnimationName, float StartTime, int32 NumLoops)
{
#if WITH_EDITOR
	VALIDATE_FLOCKS_WORLD();
#endif
	...
	ParallelFor(StateTable->Size(), [StateTable, &StateTableData, AnimationTables, AnimationName, StartTime, NumLoops](uint16 Index)
	{
		BOID_HANDLE Boid = StateTable->Get(Index);
		int32 MeshIndex = StateTableData[Index].MeshIndex;
...
void UFlocksStatics::PlayOverrideAnimationForBoidsInGroup(int32 Group, const FName AnimationName, float StartTime, int32 NumLoops)
{
#if WITH_EDITOR
	VALIDATE_FLOCKS_WORLD_AND_GROUP_INDEX(Group);
#endif
	...
	ParallelFor(StateTable->Size(), [StateTable, &StateTableData, AnimationTables, Group, AnimationName, StartTime, NumLoops](uint16 Index)
	{
		if (StateTableData[Index].Group == Group)
		{
			BOID_HANDLE Boid = StateTable->Get(Index);
...
void UFlocksStatics::ClearOverrideAnimationForBoid(const FFlocksBoidBlueprintHandle& Boid)
{
#if WITH_EDITOR
	VALIDATE_FLOCKS_WORLD_AND_BOID_HANDLE(Boid);
#endif
	...
	FlocksAnimationTable* AnimationTable = World->VisualsTable[MeshIndex]->AnimationTable;
	AnimationTable->Lock();
	AnimationTable->ClearAnimationOverride(Boid.Handle);
	AnimationTable->Unlock();

	StateTable->Unlock();
}

void UFlocksStatics::ClearOverrideAnimationForBoids(const TArray<FFlocksBoidBlueprintHandle>& Boids)
{
#if WITH_EDITOR
	VALIDATE_FLOCKS_WORLD();
#endif
	...
	ParallelFor(Boids.Num(), [StateTable, &StateTableData, AnimationTables, Boids](uint16 Index)
	{
		BOID_HANDLE Boid = Boids[Index].Handle;
		uint16 BoidIndex = StateTable->Find(Boid);
		...
		if(BoidIndex != INVALID_BOID_HANDLE_INDEX)
		{
			...
			{
				FlocksAnimationTable* AnimationTable = AnimationTables[MeshIndex];
				AnimationTable->ClearAnimationOverride(Boid);
			}
		}
	});
...
void UFlocksStatics::ClearOverrideAnimationForAllBoids()
{
#if WITH_EDITOR
	VALIDATE_FLOCKS_WORLD();
#endif
	...
	ParallelFor(StateTable->Size(), [StateTable, &StateTableData, AnimationTables](uint16 Index)
	{
		BOID_HANDLE Boid = StateTable->Get(Index);
		int32 MeshIndex = StateTableData[Index].MeshIndex;
...
void UFlocksStatics::ClearOverrideAnimationForBoidsInGroup(int32 Group)
{
#if WITH_EDITOR
	VALIDATE_FLOCKS_WORLD_AND_GROUP_INDEX(Group);
#endif
	...
	ParallelFor(StateTable->Size(), [StateTable, &StateTableData, AnimationTables, Group](uint16 Index)
	{
		if (StateTableData[Index].Group == Group)
		{
			BOID_HANDLE Boid = StateTable->Get(Index);
...
void UFlocksStatics::SetMaterialCustomValueForGroup(const int32 Group, const int32 ValueIndex, const float Value)
{
#if WITH_EDITOR
	VALIDATE_FLOCKS_WORLD_AND_GROUP_INDEX(Group);
#endif
	...
	ParallelFor(StateTable->Size(), [StateTable, &StateTableData, World, ValueIndex, Value, Group](uint16 Index)
	{
		BOID_HANDLE Boid = StateTable->Get(Index);
		if (StateTableData[Index].Group == Group)
		{
			const int32 MeshIndex = StateTableData[Index].MeshIndex;
			if(MeshIndex >= 0 && MeshIndex < MAX_MESHES)
			{
				World->VisualsTable[MeshIndex]->SetMaterialCustomDataValue(Boid, ValueIndex, Value);
			}
		}
	});
}
...
TArray<FVector> UFlocksStatics::GetAllBoidLocations()
{
	...
	FlocksStateTable::FTableData* StateTableData = StateTable->Lock();
	ParallelFor(StateTable->Size(), [&Locations, StateTableData](uint16 Index)
	{
...
TArray<FVector> UFlocksStatics::GetBoidLocations(TArray<FFlocksBoidBlueprintHandle> Boids)
{
	...
	FlocksStateTable::FTableData* StateTableData = StateTable->Lock();
	ParallelFor(Boids.Num(), [&Locations, StateTable, StateTableData, Boids](uint16 Index)
	{
...
bool UFlocksStatics::GetBoidFromBlueprintID(int32 BlueprintID, FFlocksBoidBlueprintHandle& Boid)
{
	...
	memcpy(&TempHandle.Handle, &BlueprintID, sizeof(uint32));

	if (!IsValidBoidHandle(TempHandle))
		return false;

	memcpy(&Boid.Handle, &BlueprintID, sizeof(uint32));
	return true;
}
...
bool UFlocksStatics::GetBoidBlueprintID(const FFlocksBoidBlueprintHandle& Boid, int32& BlueprintID)
{
	...
	memcpy(&BlueprintID, &Boid.Handle, sizeof(uint32));
	return true;
}
...
TArray<FFlocksBoidBlueprintHandle> UFlocksStatics::GetAllBoids()
{
#if WITH_EDITOR
	VALIDATE_FLOCKS_WORLD_RETURN({});
#endif

	int32 NumBoids = GetNumBoids();
	
	TArray<FFlocksBoidBlueprintHandle> RetVal;
	RetVal.SetNum(NumBoids);
	
	for(int32 Index = 0; Index < NumBoids; ++Index)
	{
		RetVal[Index].Handle = AFlocksWorld::WorldInst->StateTable->Get(Index);
	}
...
bool UFlocksStatics::LineTraceForBoids(FVector Start, FVector End, TArray<FLineTraceForBoidsResult>& Result)
{
	...
	FThreadSafeCounter ParallelBoidCounter = FThreadSafeCounter();
	TArray<FLineTraceForBoidsResult> LineTraceResults;
	LineTraceResults.SetNum(MAX_BOIDS);

	TArray<float> AllMeshBounds;
	AllMeshBounds.SetNum(MAX_MESHES);
	for (int32 i = 0; i < MAX_MESHES; ++i)
	{
	...
	ParallelFor(GetNumBoids(), [&LineTraceResults, &ParallelBoidCounter, StateTableData, AllMeshBounds, LineDist, World, Start, End](int32 Index)
	{
...
TArray<FFlocksBoidBlueprintHandle> UFlocksStatics::GetAllBoidsInSphere(const FVector& Origin, float Radius)
{
	...
	FlocksStateTable::FTableData* StateTableData = World->StateTable->Lock();
	ParallelFor(GetNumBoids(), [World, &ParallelBoidCounter, &Boids, StateTableData, Origin, Radius](int32 Index)
		{
			BOID_HANDLE Boid = World->StateTable->Get(Index);
...
void UFlocksStatics::GetVolumeBounds(const FFlocksVolumeBlueprintHandle& Volume, FVector& BoundsCenter, FVector& BoundsExtents, float& BoundsRadius)
{
	...
	AFlocksWorld* World = AFlocksWorld::WorldInst.Get();
	uint32 VolumeIndex = World->VolumeInfoTable->Find(Volume.Handle);
