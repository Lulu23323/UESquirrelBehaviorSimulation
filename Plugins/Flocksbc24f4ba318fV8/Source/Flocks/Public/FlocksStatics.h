...
public:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Flocks")
	FFlocksBoidBlueprintHandle HitBoid;
};
...
#pragma region Boids
	...
	/// Removes the boids with the given handles
	UFUNCTION(BlueprintCallable, Category = "Flocks")
	static void BatchRemoveBoidsByHandles(const TArray<FFlocksBoidBlueprintHandle>& Handles);
...
#pragma region Animation
	UFUNCTION(BlueprintPure, Category = "Flocks")
	static bool GetAnimNameFromIndex(UFlocksAnimationData* AnimData, int32 Index, FName& OutName);

	/// Tells the boid with the handle to play the given animation, will revert back after completing (NumLoops = -1 will loop forever)
	UFUNCTION(BlueprintCallable, Category = "Flocks")
	static void PlayOverrideAnimationForBoid(const FFlocksBoidBlueprintHandle& Boid, const FName AnimationName, float StartTime = 0.0f, int32 NumLoops = -1);

	/// Tells the boids with the handles to play the given animation, will revert back after completing (NumLoops = -1 will loop forever)
	UFUNCTION(BlueprintCallable, Category = "Flocks")
	static void PlayOverrideAnimationForBoids(const TArray<FFlocksBoidBlueprintHandle>& Boids, const FName AnimationName, float StartTime = 0.0f, int32 NumLoops = -1);

	/// Tells all boids to play the given animation, will revert back after completing (NumLoops = -1 will loop forever)
	UFUNCTION(BlueprintCallable, Category = "Flocks")
	static void PlayOverrideAnimationForAllBoids(const FName AnimationName, float StartTime = 0.0f, int32 NumLoops = -1);

	/// Tells the boids in this group to play the given animation, will revert back after completing (NumLoops = -1 will loop forever)
	UFUNCTION(BlueprintCallable, Category = "Flocks")
	static void PlayOverrideAnimationForBoidsInGroup(int32 Group, const FName AnimationName, float StartTime = 0.0f, int32 NumLoops = -1);

	/// Clears the override animation for a certain boid
	UFUNCTION(BlueprintCallable, Category = "Flocks")
	static void ClearOverrideAnimationForBoid(const FFlocksBoidBlueprintHandle& Boid);

	/// Clears the override animation for a list of boids
	UFUNCTION(BlueprintCallable, Category = "Flocks")
	static void ClearOverrideAnimationForBoids(const TArray<FFlocksBoidBlueprintHandle>& Boids);

	/// Clears the override animation for all boids
	UFUNCTION(BlueprintCallable, Category = "Flocks")
	static void ClearOverrideAnimationForAllBoids();

	/// Clears the override animation for all boids in the specified group
	UFUNCTION(BlueprintCallable, Category = "Flocks")
	static void ClearOverrideAnimationForBoidsInGroup(int32 Group);
#pragma endregion
...
#pragma region Material Values
	...
	/// Sets a custom material float on all boids in the specified group. This relates to the PerInstanceCustomData in the material.
	/// If applying a large selection of values - uncheck ApplyImmediate and call ApplyBoidMaterialCustomValues when complete
	UFUNCTION(BlueprintCallable, Category = "Flocks")
	static void SetMaterialCustomValueForGroup(const int32 Group, const int32 ValueIndex, const float Value);
...
#pragma region Getters
	...
	/// Given an index, returns the boid - this allows for loops to iterate over every boid
	UFUNCTION(BlueprintPure, Category = "Flocks")
	static TArray<FFlocksBoidBlueprintHandle> GetAllBoids();
